...
def pull_problem(problem_id: str):
    ...
    try:
        ...
        console.print(
            f"[red]Problem '{problem_id}' not found in packaged dataset.[/red]"
        )
        raise typer.Exit(1)
    ...
    (target_dir / "problem.md").write_text(
        _format_problem_md(problem_data), encoding="utf-8"
    )
    (target_dir / "solution.py").write_text(
        problem_data.get("stub", ""), encoding="utf-8"
    )
    (target_dir / ".tests.json").write_text(
        json.dumps(problem_data.get("tests", []), indent=2), encoding="utf-8"
    )
...
def prep_next():
    ...
    # Get latest campaign
    cursor.execute(
        "SELECT target_company FROM campaigns ORDER BY created_at DESC LIMIT 1"
    )
    row = cursor.fetchone()
    if not row:
        console.print(
            "[red]No active campaign found. Run `dooma prep start <company>`.[/red]"
        )
        raise typer.Exit(1)
